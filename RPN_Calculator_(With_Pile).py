...
def clicMouse(event):
    """Détecte les clics de la souris dans la calculatrice"""
    global pile
    for t in TOUCHES:
        x1 = TOUCHES[t]['x1']
        x2 = TOUCHES[t]['x2']
...
def affiche_PileRPN(pile, stack):
    """Affiche les valeurs de la pile p, dans la zone prévue à cet effet.
    """
    p1 = p.Pile()
    ...
    for i in range(6):
        stack[i].config(text = '')
    i = 0
    # On écrit les valeurs dans les case, en conservant les valeurs dépilées
    # dans une pile temporaire
    while not pile.est_vide():
        valeur = pile.depiler()
        stack[i].config(text = valeur)
        p1.empiler(valeur)
        i += 1
    # On remet les valeurs dans la pile d'origine.
...
def touchePressee(t, p):
    ...
    affiche_PileRPN(p, pileRPNStack)

# Programme principal

root = tk.Tk()
root.title("RPN Calculator")
root.geometry('800x677+50+25')

image = tk.PhotoImage(file="CalcRPN.png")
pileRPN = tk.PhotoImage(file="PileRPN.png")
...
# Zone d'affichage du contenu de la pile
canevas_pile = tk.Canvas(root, width = 400, height = 677)
canevas_pile.create_image(0,0,anchor = tk.NW, image = pileRPN)
canevas_pile.pack(side = "left")
pileRPNStack = []
for i in range(6):
    pileRPNStack.append(tk.Label(canevas_pile, text = '', font = 'Verdana 30',
                        bg='#CBC366', justify= 'right'))
    pileRPNStack[i].pack()
    canevas_pile.create_window(343, 385 - 62 * i, window = pileRPNStack[i], anchor = tk.E)
